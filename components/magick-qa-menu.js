...
let MagickQAMenu = class MagickQAMenu extends LitElement {
    constructor() {
        super(...arguments);
        this.alignment = "right"; // Track alignment (left or right)
        this.isDragging = false; // Track dragging state
        this.startX = 0; // Track initial X position during drag
        this.dragDelta = 0; // Track drag distance
        ...
        this.doDrag = (event) => {
            ...
            const container = this.shadowRoot?.querySelector(".magick-qa-menu-container");
            // Move the container horizontally based on drag, but restrict it to the viewport
            const moveDistance = Math.max(Math.min(this.dragDelta, window.innerWidth), // Limit to the width of the screen
            -window.innerWidth);
...
], MagickQAMenu.prototype, "alignment", void 0);
__decorate([
    state()
], MagickQAMenu.prototype, "isDragging", void 0);
__decorate([
    state()
], MagickQAMenu.prototype, "startX", void 0);
__decorate([
    state()
], MagickQAMenu.prototype, "dragDelta", void 0);
MagickQAMenu = __decorate([
    customElement("magick-qa-menu")
], MagickQAMenu);
export { MagickQAMenu };
//# sourceMappingURL=magick-qa-menu.js.map